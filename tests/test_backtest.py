...
class BacktestTests(unittest.TestCase):
    def _run(
        self, directory: Path, *, events: list[dict[str, object]] | None = None
    ) -> CompletedBacktest:
        config_path, data_path = _write_inputs(directory, events=events)
    ...
    def test_invalid_inputs_and_insufficient_cash_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            duplicate_config.write_text(
                '{"schema_version":1,"mode":"DRY","data_sha256":"'
                ...
                '"fee_bps":"10","max_quote_age_seconds":300},"random_seed":7}',
                encoding="utf-8",
            )
            with self.assertRaises(BacktestInputError):
